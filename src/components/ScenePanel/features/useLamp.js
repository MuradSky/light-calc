...
const useLamp = (scene) => {
    ...
    function updateLamps(roomWidth, lampHeight, roomDepth, lampCount, margin = 1) {
        ...
        // Шаги между светильниками
        const xStep = usableWidth / (lampsPerRow > 1 ? lampsPerRow - 1 : 1);
        const zStep = usableDepth / (lampsPerColumn > 1 ? lampsPerColumn - 1 : 1);
        ...
        for (let row = 0; row < lampsPerRow; row++) {
            ...
            const x = -roomWidth / 2 + margin + row * xStep;
            const z = -roomDepth / 2 + margin + col * zStep;
